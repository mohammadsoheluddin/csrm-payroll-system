...
const REPORT_DEFINITIONS: TReportCenterDefinition[] = [
  ...
  {
    id: "legacy_salary_archive",
    title: "Legacy Salary Sheet Archive",
    category: "salary",
    flow: "salary",
    sequence: 46,
    description: "Imported salary sheets from old/current payroll software for historical archive, search, charts and comparison. Not mixed into native payroll calculation.",
    modulePath: "/api/v1/legacy-salary-imports",
    routePath: "/api/v1/legacy-salary-imports/summary",
    periodType: "monthly",
    supportedFormats: ["preview", "csv", "excel"],
    exportPaths: {
      preview: "/api/v1/legacy-salary-imports/summary",
      csv: "/api/v1/legacy-salary-imports/records/export/csv",
      excel: "/api/v1/legacy-salary-imports/records/export/excel",
    },
    requiredFilters: ["payrollMonth"],
    optionalFilters: ["company", "groupBy"],
    requiredPermission: PERMISSIONS.LEGACY_SALARY_IMPORT_READ,
    readinessGate: "none",
    downstreamUse: "Historical salary archive, reconciliation, analytics and comparison with native payroll data.",
    isOperationalReport: true,
  },
  {
