...
import {
  TOtStatementExportFileResult,
  TOtStatementExportPreview,
  TOtStatementExportRow,
...
const REPORT_TITLE = "OT Statement";

const sanitizeFileNamePart = sanitizeReportFileNamePart;

const formatAmount = formatReportAmount;

const formatDecimal = formatReportDecimal;

const getFileBaseName = (preview: TOtStatementExportPreview) =>
...
const getRowValue = (row: TOtStatementExportRow, key: string) => {
  const rowRecord = row as unknown as Record<string, unknown>;

  return rowRecord[key] ?? "";
};

export const generateOtStatementCsv = (
  preview: TOtStatementExportPreview,
): TOtStatementExportFileResult => {
...
export const generateOtStatementExcel = async (
  preview: TOtStatementExportPreview,
): Promise<TOtStatementExportFileResult> => {
...
const addPdfHeader = (doc: PDFKit.PDFDocument, preview: TOtStatementExportPreview) => {
...
const addPdfSummary = (doc: PDFKit.PDFDocument, preview: TOtStatementExportPreview) => {
...
const addPdfRows = (doc: PDFKit.PDFDocument, rows: TOtStatementExportRow[]) => {
...
export const generateOtStatementPDF = async (
  preview: TOtStatementExportPreview,
): Promise<TOtStatementExportFileResult> => {
